...
for (let h of document.querySelectorAll("main :is(h2, h3, h4, h5, h6):not(:is(article, section, .no-section-wrapper) *)")) {
...
let navLinks = document.querySelectorAll("#site-nav > .nav-list > .nav-list-item");

navLinks[4]?.classList.add("active");
navLinks[7]?.classList.add("active");

for (let img of document.querySelectorAll("main img")) {
...
function updateIntrinsicDimensions (img) {
	...
	if (!img.classList.contains("no-link") && figure && (naturalWidth > img.offsetWidth / 4 || naturalHeight > img.offsetHeight / 4)) {
